	...
	public boolean[][] movimentosPossiveis() {
		...
		if (getCor() == Cor.BRANCO) {
			...
			if (getTabuleiro().ExistePosicao(p) && !getTabuleiro().temPeca(p) && getTabuleiro().ExistePosicao(p2)
					&& !getTabuleiro().temPeca(p2) && getContaMovimentos() == 0) {
				mat[p.getLinha()][p.getColuna()] = true;
			}
			...
			if (posicao.getLinha() == 3) {
				...
				if (getTabuleiro().ExistePosicao(esquerda) && temUmaPecaOponente(esquerda)
						&& getTabuleiro().peca(esquerda) == partidaXadrez.getEnPassantVulneravel()) {
					mat[esquerda.getLinha() - 1][esquerda.getColuna()] = true;
				}
				Posicao direita = new Posicao(posicao.getLinha(), posicao.getColuna() + 1);
				if (getTabuleiro().ExistePosicao(direita) && temUmaPecaOponente(direita)
